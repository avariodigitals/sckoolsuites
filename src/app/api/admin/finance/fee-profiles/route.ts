...
export async function GET() {
  ...
  return NextResponse.json({
    profiles: profiles.map((p) => ({
      id: String(p.id),
      name: p.name,
      feeGroupId: String(p.feeGroupId),
      ...
      items: p.items?.map((i) => ({
        id: String(i.id),
        feeComponentId: String(i.feeComponentId),
        feeComponentName: i.feeComponent?.name,
        amount: i.amount,
        isOptional: i.isOptional,
      })) ?? [],
      classes: p.classes?.map((c) => ({ id: String(c.classId), name: c.class?.name })) ?? [],
      arms: p.arms?.map((a) => ({ id: String(a.armId), name: a.arm?.name })) ?? [],
    })),
    stats: {
